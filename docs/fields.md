...
const hero = {
  ...
  fieldDefinitions: [
    ...
    { fieldType: 'asset', valueType: 'reference', slug: 'image', min: 1, max: 1, /* ... */ },
  ],
};
...
Direct values (`string` / `number` / `boolean`) and `reference` values store per-language content; `component` values store an ordered, language-independent array of items whose internal values are themselves translatable.
