...
- **The data directory has to stay short on Windows** - Windows resolves paths against a 260 character limit unless long paths are enabled. Core stores an Entry at `projects/<uuid>/collections/<uuid>/<uuid>.json` and Git LFS stores its objects at a comparable depth, so it needs about 137 characters below the data directory. A data directory beyond roughly 120 characters therefore runs out of room and file operations fail with `ENAMETOOLONG`. Core does not validate this, because the limit depends on the platform and on whether long paths are enabled, so a fixed cap would reject setups that work. Keep `ELEK_IO_DATA_DIR` short on Windows instead, see [`usage.md`](./usage.md#environment-variables). macOS and Linux allow 1024 and 4096 characters and are not affected.
