...
- **`synchronize()` has no conflict or integrity handling** - it fetches, rebases and pushes with no guard. A textual rebase conflict surfaces as a generic `Internal` error and can leave the repository mid-rebase, and there is no guard for uncommitted local changes. A clean rebase can also integrate a dangling reference (a delete on one side combined with a new reference to it on the other) and push it to the remote without any check, the one reference-integrity case the per-operation gates cannot catch. A planned pre-push integrity gate would validate the integrated tree and stop the sync before pushing if it would dangle. See [`references.md`](./references.md) and the design note `docs/design/sync-dangling-reference-prevention.md`.
...
- **Delete protects Assets, Entries and Collections from dangling references** - deleting an Asset or Entry that is still referenced by another Entry's values is blocked with a `Conflict` error, mirroring Component delete protection. Deleting a Collection is blocked the same way when a surviving Entry outside it still references into it, so removing a whole Collection cannot strand references either. References are detected in flat `reference` fields, in `assetReference` / `entryReference` nodes inside `mdast` fields, and nested inside `dynamic` / component items. The `Conflict` carries the list of referring Entries (each with the offending field and, for nested cases, the component path) so an editor can link straight to them. References between Entries that are all being deleted together, including an Entry that references only itself, do not block, because they vanish cleanly. Enforcement scans every Entry in the Project on each delete rather than using a persisted reverse index, which keeps it correct against Entries brought in by a pull or merge but costs one read per existing Entry per delete, so it scales poorly for very large Projects. Only the current `work` tree is considered, not entities preserved in released (`production`) history. A direct reference to a Collection as a whole is detected too, as a defensive measure, although no field type produces one and no supported workflow creates one.
- **Reference protection ignores field `defaultValue`s** - an Asset or Entry reference embedded in a Collection or Component field's markdown `defaultValue` is not counted as a reference, because it only becomes a live reference once stamped into an Entry.
...
- [`references.md`](./references.md) - how references work end to end, including write-time and delete-time integrity
